...
export const POST = async (request: Request) => {
  try {
    ...
    const htmlTemplate = `
      <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
        ...
        <footer style="margin-top: 30px; font-size: 12px; color: #aaa; text-align: center;">
          Pesan ini dikirim via sistem otomatis Portfolio Muhamad Taufiq Riza.
        </footer>
      </div>
    `;
    ...
    try {
      const { repoCreate } = await import("@/services/admin/repository");
      await repoCreate("contact_messages", {
        name,
        email,
        message,
        is_read: false,
